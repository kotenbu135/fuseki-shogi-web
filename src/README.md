...
**両玉を置くあいだ**（置く人の番）も `#king-tags` を使う。置く陣の境目に「先手玉をここに · 薄く」の札（`.zone-hint`）、
置いた玉に「先手玉」の札、駒台の玉に輪郭（`.kings-only`）。状態欄には天秤の図（`#scale-fig`。左の皿が先手＝先に動ける得＋薄い玉、
右の皿が後手＝堅い玉、梁は水平）を玉を置く・先後を選ぶあいだだけ出す。文で「先手玉は薄く」と言うだけでは、
自分の玉のつもりで先手玉を堅く置いてしまう人がいた（初めての人が実際にそう置いた）。釣り合っているかの判定（傾き）は出さない。
SVG 要素には `.hidden` が無い（HTMLElement だけ）ので、図の出し入れは `toggleAttribute('hidden')`。
...
候補手は1本1行（評価・手・読み筋を同じ行に、読み筋は省略記号で切る）、布石の候補は札を横に並べる（`.candidates.chips`）。
検討では状態・操作2行・グラフ・候補手が棋譜と高さを分けるので、1本を2行にすると 1920×1080 で棋譜が3行に潰れた。
広い画面の棋譜の行は `minmax(9rem, 1fr)` で、足りなければパネルが盤より高くなる方を選ぶ。
