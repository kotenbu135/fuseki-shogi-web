...
ホームで選べる**玉分け将棋**（英語名 King Split Shogi）は、この差を対局者自身が毎局その場で
打ち消す手順。

1. 振り駒で**玉を置く役**を決める。もう一方が**選ぶ役**。
2. 置く役が先手玉を先手陣に、続けて後手玉を後手陣に置く（1・2手目）。
3. 選ぶ役が、先手を持つか後手を持つかを決める。ここで先後が決まる。画面では盤上の玉を押し、
   盤が回ってから「後手を持って始める」で確定する（二段）。
4. 残りの19枚ずつを先手から交互に置き、41手目から通常の将棋。

置く役は自分がどちらの色になるか知らずに両玉を置くので、先手玉を少し悪いマスに置いて
釣り合わせるしかない。ケーキを切る人と選ぶ人を分ける仕組み（pie rule）と同じで、
強さが変わっても釣り合いが自動で追従する。置く範囲・禁じ手・41手目の裁定は元のまま。
経緯は対局サイトの「玉分け将棋について」（`src/pages/*/story.html`）に書いてある。

AIの置く役と選ぶ役は、両玉のマスの組ごとの先手勝率の表（[models/README.md](models/README.md)）を
引くだけで、探索はしない。手順の文字列では2手目の後に `choose:sente` / `choose:gote` が挟まる。

## 画面と言語

- **ホーム**（`#/`）でルール（布石将棋／玉分け将棋）と設定を選んで始め、**対局画面**（`#play`）で指す。
  右のパネルは席・フェーズ帯・状態・操作・棋譜・席の固定スロットで、状態が変わっても
  棋譜の高さしか動かない（`test/browser_smoke.mjs` の `layoutCheck` が操作のたびに測る）。
...
python3 test/policy_parity.py policy_probe.json models/fuseki_degct_b3_iter122.onnx
...
# 玉分け将棋の状態機械（kings → choose → fuseki、選択をまたぐ待った、手順の往復）
