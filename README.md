...
置く範囲・二歩回避の禁じ手・41手目の裁定は元のまま。
3〜40手目には天秤将棋だけの禁じ手**二飛香**（自陣の同じ筋に自分の飛・香は合わせて1枚まで。2026-09-13 に追加、
[Issue #1](https://github.com/kotenbu135/fuseki-shogi-web/issues/1)）が掛かる。cppshogi の `FusekiRuleNihikyo` を
`Game` が天秤将棋のときだけ `fuseki.reset()` に渡す（`Game.fusekiRules`）。布石将棋は shogitter のルールのまま。

手順の文字列にルールの版は書かない。**版の無い天秤将棋の手順は、いまの版で入らず二飛香の前の版なら入るとき、
旧ルールの棋譜としてそちらで再生し、読み込み時にそう知らせる**（`src/game.js` の `rulesForTokens`）。
両方の版で入る手順はどちらで再生しても同じ盤・同じ結果になるので、この判定で取り違えは起きない。
両玉の価値表・帯・布石方策（iter2455）は二飛香の前に測った・学習したもので、測り直していない。
方策は `legalDrops()` でマスクしているので、二飛香に当たる手を除いた残りから選ぶ。
...
- **ホーム**（`#/`）で相手（AI／友達／待合）を選び、ルール（布石将棋／天秤将棋）と設定を選んで始め、**対局画面**（`#play`）で指す。
  相手のタブで要る設定だけが出る（AIの強さはAIだけ、名前はオンラインだけ）。最後のタブを覚え、`#lobby` で待合タブを直接開ける。
...
- **友達と対局**（ホームの「友達と対局」タブ、「招待リンクを作る」）は部屋を作って招待リンク（`#room/<id>`）を渡す。
...
- **ルール**（`/rules/`）・**天秤将棋の案内**（`/balance/`）・**コラム**（`/story/`）は `src/pages/<lang>/` の
  中身を `src/page.html` の殻に入れて出す。コラムのヒートマップは公開している価値表をそのまま読む。
  案内の「天秤将棋で対局する」は `/#balance` で、ホームが天秤将棋の札を選んだ状態で開く。
  検索エンジン向けに、トップと案内には JSON-LD（WebSite・VideoGame）を入れている。
...
- **天秤将棋のルールの版は部屋が持つ**（state `v: 3` の `balanceRules`）。部屋は作った時点の版で終局まで指し、
  判定役もブラウザもその版で `Game` を作る。版を持たない部屋（`v: 2` まで。二飛香の前に作られた部屋）は
  旧ルールのまま判定するので、Worker を deploy した時点で進行中の天秤将棋の部屋もそのまま続けられる。
  手順は **Worker を先に deploy してからサイトを push** する（逆だと、新しいサイトが古い Worker の部屋を旧ルールとして扱い、
  二飛香の無い対局が作られ続ける）。
- **待合**（`worker/src/lobby.js`）は公開で募集している部屋の一覧を1つの Durable Object が持ち、ホームの
  「待合で対局」タブへ WebSocket で流す（タブの札に募集の数）。募集は同じタブの「待合に出す」（公開の部屋。招待リンクも付く）。作った人が居るあいだだけ載り、相手が来ると外れる。名前（ニックネーム）は席に付く。
...
python3 test/policy_parity.py policy_probe.json models/fuseki_degct_b3_iter2455.onnx
