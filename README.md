...
対局画面の「ルール」で選べる**両玉先置き**は、この差を対局者自身が毎局その場で打ち消す手順。

1. 振り駒で**玉を置く役**を決める。もう一方が**選ぶ役**。
2. 置く役が先手玉を先手陣に、続けて後手玉を後手陣に置く（1・2手目）。
3. 選ぶ役が、先手側で指すか後手側で指すかを選ぶ。ここで先手・後手が決まる。
...
# 両玉先置きの状態機械（kings → choose → fuseki、選択をまたぐ待った、手順の往復）
node test/kings_first_test.mjs
# 同じことを実ブラウザで、置く役と選ぶ役の両方（models/ に価値表が要る）
